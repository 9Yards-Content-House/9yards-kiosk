-- ============================================================
-- 014: KIOSK ALIGNMENT & ENHANCEMENT
-- Key changes:
-- 1. Update order_status enum: remove 'ready', 'delivered', add 'out_for_delivery', 'arrived'
-- 2. Add 'reception' to user_role enum
-- 3. Add picked_up tracking columns
-- 4. Add location_id for multi-kiosk support
-- 5. Create locations table for future expansion
-- 6. Add delivery_fee column
-- ============================================================

-- ============================================================
-- 1. UPDATE ORDER STATUS ENUM
-- PostgreSQL doesn't support DROP from enum directly, so we need to:
-- - Add new values first
-- - Update existing data
-- - Note: Can't remove old values without recreating the type (risky)
-- ============================================================

-- Add new status values (safe - doesn't affect existing data)
...
-- ============================================================
-- 5. CREATE LOCATIONS TABLE (Multi-kiosk prep)
-- ============================================================
...
-- ============================================================
-- 6. UPDATE RLS POLICIES FOR RECEPTION ROLE
-- ============================================================
...
CREATE POLICY "Staff can view orders"
  ...
  USING (
    -- Allow anonymous order tracking by order_number (handled by existing policy)
    true
  );
...
-- Reception can mark orders as picked up
...
-- ============================================================
-- 7. UPDATE NOTIFICATION TRIGGERS FOR NEW STATUSES
-- ============================================================

-- Update the order status change notification
...
-- ============================================================
-- 8. UPDATE LOYALTY POINTS FOR NEW STATUS
-- Change from 'delivered' to 'arrived' for loyalty points
-- ============================================================
...
-- ============================================================
-- 9. HELPER FUNCTION: Get operating hours
-- ============================================================
...
BEGIN
  ...
  -- If no location found, assume open
...
-- ============================================================
-- 10. ARRIVAL NOTIFICATION TRIGGER (for WhatsApp)
-- ============================================================

CREATE OR REPLACE FUNCTION public.trigger_arrival_notification()
RETURNS trigger AS $$
BEGIN
  -- When order changes to 'arrived', log for WhatsApp notification
  IF NEW.status = 'arrived' AND OLD.status != 'arrived' AND NEW.customer_phone IS NOT NULL THEN
    -- This will be picked up by an edge function webhook or pg_notify
...
-- Create trigger for arrival notification
...
-- ============================================================
-- 11. ORDER CANCELLATION FUNCTION
-- Only allow cancellation if order is still 'new'
-- ============================================================
